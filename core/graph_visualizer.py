...
def load_graph_tables(stage2_output_dir: str | Path) -> tuple[pd.DataFrame, pd.DataFrame, dict[str, Any]]:
    ...
    metrics = json.loads(
        (base / "semantic_attribute_graph_metrics.json").read_text(encoding="utf-8")
    )
...
def build_networkx_graph(nodes: pd.DataFrame, edges: pd.DataFrame) -> nx.Graph:
    ...
    for _, node in nodes.iterrows():
        graph.add_node(
            ...
            sales_rows=float(node.get("sales_rows", 0) or 0),
            purchases_rows=float(node.get("purchases_rows", 0) or 0),
        )
...
def product_activity_score(graph: nx.Graph, product_node: str) -> float:
    data = graph.nodes[product_node]
    return float(data.get("sales_rows", 0) or 0) + float(data.get("purchases_rows", 0) or 0)


def select_focus_product_attribute_graph(
    graph: nx.Graph,
    top_attribute_count: int = 16,
    ...
    top_attrs = sorted(attribute_nodes, key=lambda node: graph.degree(node), reverse=True)[
        :top_attribute_count
    ]
    ...
    for attr in top_attrs:
        candidate_products.update(
            neighbor
            for neighbor in graph.neighbors(attr)
...
def build_attribute_projection(
    graph: nx.Graph,
    max_attribute_nodes: int = 85,
...
def layered_attribute_layout(graph: nx.Graph) -> dict[str, tuple[float, float]]:
...
def draw_attribute_projection(graph: nx.Graph, output_path: str | Path) -> str:
    ...
    legend = [
        mpatches.Patch(color=NODE_COLORS[node_type], label=node_type)
        for node_type in LAYER_ORDER
    ]
...
def draw_focus_product_attribute_graph(graph: nx.Graph, output_path: str | Path) -> str:
...
def draw_attribute_family_graph(
    graph: nx.Graph,
    output_path: str | Path,
    center_nodes: list[str] | None = None,
) -> str:
    ...
    return draw_focus_product_attribute_graph(subgraph, output_path)
...
def render_graph_visualizations(stage2_output_dir: str | Path, output_dir: str | Path) -> dict[str, str]:
    ...
    paths = {
        "g_attr_attribute_projection.png": draw_attribute_projection(
            ...
            output / "g_attr_attribute_projection.png",
        ),
        "g_attr_product_attribute_focus.png": draw_focus_product_attribute_graph(
            focus_graph,
            output / "g_attr_product_attribute_focus.png",
        ),
        "g_attr_frasco_vidrio_ambar.png": draw_attribute_family_graph(
            graph,
            output / "g_attr_frasco_vidrio_ambar.png",
        ),
    }

    manifest_path = output / "visualization_manifest.json"
    manifest = {
        "generated_from": str(Path(stage2_output_dir)),
        "graph_name": metrics.get("graph_name", "G_attr"),
        "graph_full_name": metrics.get("graph_full_name", "semantic_attribute_graph"),
        "layout": "static_networkx_png",
        "outputs": paths,
        "notes": [
            "No HTML interactivo.",
            "No fisica ni movimiento.",
            "La proyeccion de atributos conecta atributos que aparecen juntos en productos.",
        ],
    }
